...
uint64_t GetCurrentTime(const time_point<high_resolution_clock>* startTime){
	...
	, m_ConsoleProg(consoleProgram)
{
	...
	if (!m_ConsoleProg && m_CurrRomSize > rom_size)
	{
		std::cerr << "Rom is too big";
		exit(EXIT_FAILURE); //TODO: causes mem leaks
	}
	...
	//initialize CPU
	m_Cpu->pc = m_ProgramStart;
	m_Cpu->sp = 0;//stack_start;
}
...
void i8080Emulator::ThrottleCPU(uint64_t& lastThrottle, uint64_t clocksPerMs, uint64_t& compensation, steady_clock::time_point startTime, uint64_t currentTime) const {

	if (currentTime - lastThrottle <= 4000) { // Check every 4 ms
		if (m_Cpu->clockCount >= (clocksPerMs << 2) + compensation) { //throttle CPU
			const uint64_t msToSleep = 4000 - (currentTime - lastThrottle); //sleep for the rest of the 4 ms
			std::this_thread::sleep_for(microseconds(msToSleep));
			lastThrottle = GetCurrentTime(&startTime);
			m_Cpu->clockCount = 0;

			compensation = (clocksPerMs * ((lastThrottle - currentTime) - msToSleep)) / 1000;
		}
	}
...
void i8080Emulator::Start() {

	auto* pKeyboard = new Keyboard();

	auto* pDisplay = new Display("Intel 8080", 224, 256, 2);
	...
	while (!m_Cpu->halt)
	{
		...
		if (!m_ConsoleProg) {
			const uint64_t currentTime = GetCurrentTime(&startTime);

			ThrottleCPU(lastThrottle, clocksPerMs, clockCompensation, startTime, currentTime);

			pDisplay->Update(currentTime, m_Memory + stack_start, this);

			pKeyboard->Update(currentTime, m_Cpu);
		}
...
//#endif
	...
	//TODO: check if print debug is still needed
	//if (m_Cpu->printDebug)
	//{
	...
	if (m_ConsoleProg) {
		switch (m_Cpu->pc) {
		case 0: Syscall(m_Cpu->pc); break; // Exit
		case 5: Syscall(m_Cpu->pc); break; // Print Text
		}
	}
}
...
void i8080Emulator::Syscall(uint16_t ID) {
	...
	case 5:
		if (m_Cpu->c == 2) {
			printf("%c", m_Cpu->e); //TODO: remove printf
		}
		else if (m_Cpu->c == 9) {
			for (int i = m_Cpu->ReadRegisterPair(RegisterPairs8080::DE); m_Memory[i] != 0x24; i++) {
				printf("%c", m_Memory[i]);
			}
		}
		fflush(stdout);
...
void i8080Emulator::MemWrite(uint16_t address, uint8_t data)
{
	...
	if (address < m_CurrRomSize) { //TODO: redo tests to see if it doesnt crash here AND TO GET SCREENSHOTS
...
void i8080Emulator::RAL() {
	...
	m_Cpu->a <<= 1;
	m_Cpu->a = m_Cpu->a | uint8_t(m_Cpu->ConditionBits.c);
...
void i8080Emulator::SUB() {
	...
	uint16_t sum = m_Cpu->a + m_Cpu->ReadRegister(reg) + 1;//(1-(uint8_t)m_Cpu->ConditionBits.c);
	m_Cpu->ConditionBits.ac = (sum ^ m_Cpu->a ^ m_Cpu->ReadRegister(reg)) & (1 << 4);
	m_Cpu->ConditionBits.c = sum > 0xFF;

	m_Cpu->a -= m_Cpu->ReadRegister(reg);
	m_Cpu->pc += 1;
...
void i8080Emulator::SBB() {
	...
	uint16_t result = m_Cpu->a - (m_Cpu->ReadRegister(reg) + m_Cpu->ConditionBits.c);

	uint16_t sum = m_Cpu->a + m_Cpu->ReadRegister(reg) + (1 - (uint8_t)m_Cpu->ConditionBits.c);
	m_Cpu->ConditionBits.ac = (sum ^ m_Cpu->a ^ m_Cpu->ReadRegister(reg)) & (1 << 4);
	m_Cpu->ConditionBits.c = sum > 0xFF;
