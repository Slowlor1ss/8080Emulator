...
private:
	void ThrottleCPU(uint64_t& lastThrottle, uint64_t clocksPerMs, uint64_t& compensation,
					std::chrono::steady_clock::time_point startTime, uint64_t currentTime) const;
	void CycleCpu();
	...
	static constexpr int memory_size = 0x10000; //TODO: make lowercase uppercase if for macro's
